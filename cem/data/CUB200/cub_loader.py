...
class CUBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.path_transform == None:
            img_path = os.path.join(self.root_dir, img_path.split("datasets/")[-1])
...
def compute_topk_concepts_of_classes(
    *,
    ...
    split="train",
    topk_list=(4, 8, 16),
    output_dir="cem/data/cocnepts_of_classes/cub/",
    use_uncertain_label=False,
):
    """Compute per-class top-k most frequent concepts for CUB200.

    Maintains a count matrix M with shape (C=200, K=112) where concepts are the
    112 attributes in SELECTED_CONCEPTS. Also maintains T with shape (C,) for
    the number of samples per class.

    This function reads the preprocessed metadata directly from:
        {root_dir}/class_attr_data_10/{split}.pkl
    to avoid image loading.

    Class names are read from:
        cem/data/CUB200/CUB_200_2011/classes.txt
    where each line is like: "1 001.Black_footed_Albatross".

    For each k in topk_list, saves:
        - {output_dir}/top{k}_indices.csv  (rank 1..k per concept column)
        - {output_dir}/top{k}_freqs.csv    (frequency per concept column)
    Columns are concept names from CONCEPT_SEMANTICS[SELECTED_CONCEPTS].

    Returns:
        (M, T): torch tensors with shapes (200,112) and (200,)
    """
    if root_dir is None:
        root_dir = DATASET_DIR
    ...
    if not os.path.exists(pkl_path):
        raise FileNotFoundError(f"Could not find split PKL at {pkl_path}")

    # Load class names (second token per line)
    classes_path = os.path.join("cem", "data", "CUB200", "CUB_200_2011", "classes.txt")
    if not os.path.exists(classes_path):
        classes_path = os.path.join(root_dir, "CUB_200_2011", "classes.txt")
    if not os.path.exists(classes_path):
        raise FileNotFoundError(
            f"Could not find classes.txt at {os.path.join('cem','data','CUB200','CUB_200_2011','classes.txt')} or {os.path.join(root_dir,'CUB_200_2011','classes.txt')}"
        )
    with open(classes_path, "r", encoding="utf-8") as f:
        label_names = [line.strip().split(maxsplit=1)[1] for line in f if line.strip()]
    if len(label_names) != N_CLASSES:
        raise ValueError(f"Expected {N_CLASSES} class names but got {len(label_names)} from {classes_path}")

    concept_names = [CONCEPT_SEMANTICS[i] for i in SELECTED_CONCEPTS]
    K = len(concept_names)

    with open(pkl_path, "rb") as f:
        samples = pickle.load(f)

    M = torch.zeros((N_CLASSES, K), dtype=torch.long)
    T = torch.zeros((N_CLASSES,), dtype=torch.long)

    # Detect whether labels are 1-indexed in PKL
    max_label = max(int(s["class_label"]) for s in samples)
    label_offset = 1 if max_label == N_CLASSES else 0

    attr_key = "uncertain_attribute_label" if use_uncertain_label else "attribute_label"

    for s in samples:
        y = int(s["class_label"]) - label_offset
        if y < 0 or y >= N_CLASSES:
            continue
        attr = np.array(s[attr_key])
        # Keep only selected concepts
        if attr.shape[0] != K:
            if attr.shape[0] > max(SELECTED_CONCEPTS):
                attr = attr[SELECTED_CONCEPTS]
            else:
                raise ValueError(f"Unexpected attribute length {attr.shape[0]} (expected {K})")
        M[y] += torch.as_tensor(attr, dtype=torch.long)
        T[y] += 1
    ...
    for k in topk_list:
        ...
        top_counts, top_indices = torch.topk(M.to(dtype=torch.float32), k_eff, dim=1, largest=True)
        top_freqs = top_counts / T_safe.unsqueeze(1)

        rank_matrix = [["" for _ in range(K)] for _ in range(N_CLASSES)]
        freq_matrix = [[0.0 for _ in range(K)] for _ in range(N_CLASSES)]
...
if __name__ == "__main__":
    dataset_name = "cub"
    if dataset_name == "cub":
        m, t = compute_topk_concepts_of_classes(split='train', root_dir=os.path.join(DATASET_DIR, "CUB_200_2011"), topk_list=(8,), output_dir="cem/data/cocnepts_of_classes/cub/")
    else:
